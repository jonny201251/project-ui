...
export default (props) => {
  ...
  useEffect(async () => {
    ...
    logicFlow.on('edge:dbclick', ({ data: edge }) => {
      //连线的开始不能是开始事件和并行网关
      if (
        edge.sourceNodeId.indexOf('StartEvent') < 0 &&
        edge.sourceNodeId.indexOf('ParallelGateway') < 0
      ) {
  ...
  const onClick = async () => {
    ...
    const dataa = await post(
      data.processDesign.id ? processDesignPath.edit : processDesignPath.add,
      data,
    );
    if (dataa) {
      ...
      <Form form={form}>
        <SchemaField>
          <SchemaField.Void
            ...
            x-component-props={{ maxColumns: 5, strictAutoFit: true }}
          >
            <SchemaField.String
              name="name"
              required
