...
export default () => {
  ...
  const render = (record) => {
    return <a onClick={async () => {
      const data = await get(projectInOutPath.get, { projectId: record.projectId })
      if (data) {
        let dialog = FormDialog({ title: '项目收支', footer: null, keyboard: false, maskClosable: false, width: 1000 },
          (form) => {
            form.setValues(data)
  ...
  return <ProTable
    ...
    //列表数据
    params={{ list: projectIn1Path.list }}
