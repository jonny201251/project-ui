...
export default () => {
  const columns = [
    {
      title: '项目类别', dataIndex: 'projectType', valueType: 'text', hideInSearch: true,
      valueEnum: {
        一般项目: { text: '一般项目' },
        重大项目: { text: '重大项目' },
        一般项目非: { text: '一般项目非' },
    ...
    { title: '项目名称', dataIndex: 'name', valueType: 'text', width: '20%', fixed: true },
    ...
    { title: '合同编号', dataIndex: 'contractCode', valueType: 'text' },
    {
      ...
      render: (text, record, _, action) => {
        return record.contractMoney > 0 ? (record.contractMoney + '').replace(/\B(?=(\d{3})+(?!\d))/g, ',') : ''
      },
    },
    { title: '合同名称', dataIndex: 'contractName', valueType: 'text', ellipsis: true },
